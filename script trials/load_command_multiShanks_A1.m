...
experimentName = '2020-09-30_11-14-36'
sessionName = 'V1_20200930_1'
...
recordingDepth = -380; % !!! Modify for each experiment !!!
channelNo = 16;
probe = '2x16_E1';
animal.name = '20200924_RV1';
animal.sex = 'f';
animal.strain = 'PvCre';
...
for i=(1:channelNo)
    filename = ['100_CH', num2str(i), '.continuous'];%+ch_offset
...
% for i=1:3%(17:19)
%     filename = ['100_AUX', num2str(i), '.continuous'];%+ch_offset
%     [data(i,:), timestamps(:,1), info(:,i)] = load_open_ephys_data([basePathData, filesep, filename]);
%     % calculate median over each channel and subtract from channel
%     med(i) = median(data(i,:));
%     std_ch(i) = std(data(i,:));
...
selCh = 1;%+ch_offset;  % selected channel for figure and calculation 
...
% chunk = 1024;
% 
% if sessionInfo.nShanks == 2
%     datavector = nan(2, numel(range1)*channelNo/sessionInfo.nShanks);
%     
%     for i =0:(numel(range1)/chunk)-1
%         dpBeg = channelNo/sessionInfo.nShanks*chunk*i+1;
%         dpEnd = channelNo/sessionInfo.nShanks*chunk*(i+1);
%         datavector(1,dpBeg:dpEnd) = reshape(gain*data_filt(channelNo/sessionInfo.nShanks+1:channelNo,1:chunk),1,[]);  % max ist 32000
%         datavector(2,dpBeg:dpEnd) = reshape(gain*data_filt(1:(channelNo/sessionInfo.nShanks),1:chunk),1,[]); 
%         data_filt = data_filt(:,chunk+1:end);
% %         datavector(2,:) = reshape(gain*data_filt(1:(channelNo/sessionInfo.nShanks),:), [1, numel(range1)*(channelNo/sessionInfo.nShanks)]);  % max ist 32000
%     end
% end
...
if sessionInfo.nShanks == 2
    datavector(1,:) = reshape(gain*data_filt((channelNo/sessionInfo.nShanks+1:channelNo),:), [1, numel(range1)*(channelNo/sessionInfo.nShanks)]);  % max ist 32000
%     data_filt = data_filt(1:(channelNo/sessionInfo.nShanks),:);
%     datavector(2,:) = reshape(gain*data_filt(1:(channelNo/sessionInfo.nShanks),:), [1, numel(range1)*(channelNo/sessionInfo.nShanks)]);  % max ist 32000
end
%%
%clearvars -except datavector sessionInfo timeSeries
max(max(datavector))
sum(sum(datavector>32768)) % should not be larger than 1 000 x channelNo
min(min(datavector))
sum(sum(datavector<-32768)) % should not be larger than 1 000 x channelNo
...
for i = 1%:sessionInfo.nShanks
    datFilename = [basePathKlusta, filesep, num2str(i), filesep, sessionName, '.dat']
    if exist(datFilename,'file')
        warning('.dat file already exists.')
    else
        newsubfolderKlusta = fullfile(basePathKlusta, num2str(i));
        mkdir(newsubfolderKlusta); % make Klusta subfolder
        f = fopen(datFilename, 'w');
        fwrite(f, datavector(i,:), 'int16');
        fclose(f);
    end
end
