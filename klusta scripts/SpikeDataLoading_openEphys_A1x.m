...
%%% modified 18.11.2021 by Ruxandra %%%

%%% Keep only the spikes after photostimulation
...
if ~SCDexist
    ...
    spikeClusterData.times = double(hdf5read(filename_kwik, '/channel_groups/0/spikes/time_samples'))/sessionInfo.rates.wideband; % all spike times
...
for condInt = [2,4]%1:totalConds % for all conditions
    ...
    for trialInt=1:tempTotalTrials % for every trial
        ...
        startTime= refTime + sessionInfo.optStimInterval(1); % startTime could be seconds before the real condition start
        ...
        codesExcl = [codesExcl; trialCodes];
...
spikeClusterData.timesExcl = timesExcl;
spikeClusterData.codesExcl = codesExcl;
