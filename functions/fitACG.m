function fit_params_out = fitACG(acg_narrow, clusterIDs, plots)
...
if plots
    ...
    for j = 1:size(acg_narrow,2)
        ...
        if ~any(isnan(acg_narrow(:,j)))
            ...
            title(ha(jj),[num2str(clusterIDs(j)) , ': rise=' num2str(fit_params(2,j),3),', decay=' num2str(fit_params(1,j),3),])
            xlim(ha(jj),[0,50])
        end
        jj = jj + 1;
    end
