...
for cond = 1:totalConds-2
    for unit = find(iUnitsFilt)%find(amplSelect)%
        normTraceFreqAll100Adj(cond, unit, :) = normTraceFreqAll(cond, unit, :) / corrF;
    end
    meanNormTraceFreqAll100Adj(cond, :) = squeeze(nanmean(normTraceFreqAllAdj(cond,:,:),2));
end
...
for cond = 1:2:totalConds
    for unit = find(baseSelect)
        for stim = 1:totalStim
            if abs(allStimMagnNormTraceFreqAllAdj(1, unit, stim))>=0.001
