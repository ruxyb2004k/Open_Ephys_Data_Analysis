...
% Fig 1b (2x): average of time courses evoked activity 100% contrast and spontaneous activity

if totalStim == 6
    titleFig1b = {'100% visual stim. vs 100% visual + photostim. all cells',...
    '0% visual stim. vs 0% visual + photostim. all cells'};
    saveFig1b = {'meanTC100All.fig', 'meanTC0All.fig'};
elseif totalStim == 1
    titleFig1b = {'100% visual stim. vs 100% visual + photostim. all cells',...
    ...
    saveFig1b = {'meanTC100All.fig', 'meanTC50All.fig','meanTC25All.fig','meanTC12All.fig','meanTC0All.fig'};
end
...
%     plot((plotBeg:bin:plotEnd), meanTraceFreqAll(cond+1,:),'LineWidth', 1, 'Color', C(cond+1,:)); hold on
    ...
    if saveFigs == true
        savefig(strcat(savePath, saveFig1b{(cond+1)/2}));
        title('');
        saveas(gcf, strcat(savePath, saveFig1b{(cond+1)/2}(1:end-3), 'png'));
        saveas(gcf, strcat(savePath, saveFig1b{(cond+1)/2}(1:end-4)), 'epsc');
    end
end
