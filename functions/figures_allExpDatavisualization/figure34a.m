...
% Fig 34a (5x): Violin plots of Opto-index with median and quartiles - 
% comparison magnitudes between before and during photostim. 


titleFig34a = {'Opto-index 100% visual stim. +/- photostim. Magn1',...
...
saveFig34a = {'OptoindexViolinplot100Magn1Class.fig',...
    'OptoindexViolinplot100Magn2Class.fig', 'OptoindexViolinplot100Magn3Class.fig',...
    'OptoindexViolinplot100Magn4Class.fig', 'OptoindexViolinplot100Magn5Class.fig',...
    'OptoindexViolinplot100Magn6Class.fig'};
...
for cond = 1%(1:2:totalConds)
    for stim =[1,4]%2:totalStim
        val = squeeze(OIndexAllStimMagn((cond+1)/2,:, stim))';
        ...
        title(titleFig34a{stim},'FontSize',18);
...
%         set(gca,'FontSize',24, 'XTickLabel',{'Without photostim.','With photostim.'},'XTick',[1 2]);
        ...
        yl = ylim;
        p_temp = pOIndexAllStimMagnExcInh;
        ...
        box off
        if saveFigs == true
            savefig(strcat(savePath, saveFig34a{stim}));
            title('');
            saveas(gcf, strcat(savePath, saveFig34a{stim}(1:end-3), 'png'));
            saveas(gcf, strcat(savePath, saveFig34a{stim}(1:end-4)), 'epsc');
            writetable(table_data1, strcat(savePath, saveFig34a{stim}(1:end-3), 'xlsx'),'Sheet',1, 'Range','A:C')
            writetable(table_data2, strcat(savePath, saveFig34a{stim}(1:end-3), 'xlsx'),'Sheet',1, 'Range','D:F')
