...
% Fig. 30bx (2x) : Bar plot, difference of magnitude between the normalized
% traces  (magn =1 and not peak =1)

if totalStim == 6
    titleFig30bx = {'Magnitude in normalized difference trace 100%',...
        'Magnitude in normalized difference trace 0%'};
    
    saveFig30bx = {'meanAllStimMagnNormTraceFreqAllAdjSubtr100Subtr.fig','meanAllStimMagnNormTraceFreqAllAdjSubtr0Subtr.fig'};
elseif totalStim ==1
    titleFig30bx = {'Magnitude in normalized difference trace 100%',...
    ...
    saveFig30bx = {'meanAllStimMagnNormTraceFreqAllAdjSubtr100Subtr.fig', 'meanAllStimMagnNormTraceFreqAllAdjSubtr50Subtr.fig','meanAllStimMagnNormTraceFreqAllAdjSubtr25Subtr.fig','meanAllStimMagnNormTraceFreqAllAdjSubtr12Subtr.fig','meanAllStimMagnNormTraceFreqAllAdjSubtr0Subtr.fig'};
end
...
for cond = (1:2:totalConds-2)
    ...
    ax = gca;
    hold on
    b4b =bar((1:totalStim)/fw, meanAllStimMagnNormTracesBaseSubtr100Subtr((cond+1)/2,:), 'EdgeColor', 'none', 'BarWidth', 0.6/fw); hold on
    b4b.FaceColor = 'flat';
%     b4b.CData = C(cond+1,:);
    b4b.CData = cCreCellType;
    errorbar((1:totalStim)/fw,meanAllStimMagnNormTracesBaseSubtr100Subtr((cond+1)/2,:),STEMallStimMagnNormTracesBaseSubtr100Subtr((cond+1)/2,:), '.','Color', cCreCellType,'LineWidth', 4); hold on
    xlabel('Stimulus no.');
...
%     
    if saveFigs == true
        savefig(strcat(savePath, saveFig30bx{(cond+1)/2}));
        title('');
        saveas(gcf, strcat(savePath, saveFig30bx{(cond+1)/2}(1:end-3), 'png'));
        saveas(gcf, strcat(savePath, saveFig30bx{(cond+1)/2}(1:end-4)), 'epsc');
    end
end