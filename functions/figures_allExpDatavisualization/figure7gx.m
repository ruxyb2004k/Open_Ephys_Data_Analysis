...
if totalStim == 6
    titleFig7gx = {'PDF OI 100% visual stim. +/- photostim. Base2',...
    ...
    saveFig7gx = {'PDFOI100Base2.fig', 'PDFOI100Base3.fig',...
...
for cond = (1:2:totalConds)
    for stim = 2:numel(baseStim)
        ...
        yyaxis right
        plot(OIvalues,squeeze(distOIndexAllStimBaseExc((cond+1)/2, stim,:)), '-g', 'LineWidth', 2); hold on
        plot(OIvalues,squeeze(distOIndexAllStimBaseInh((cond+1)/2, stim,:)), '-r', 'LineWidth', 2);
        ...
        histogram(OIndexAllStimBase((cond+1)/2,excUnits, stim),edges, 'FaceColor', EI_Color(1,:), 'FaceAlpha', 1); hold on
        histogram(OIndexAllStimBase((cond+1)/2,inhUnits, stim),edges, 'FaceColor', EI_Color(2,:), 'FaceAlpha', 1);
%         histogram(OIndexAllStimBase((cond+1)/2,excUnits, stim),edges, 'FaceColor', EI_Color(1,:), 'FaceAlpha', 1); hold on
        bar(edges(1:end-1)+binEdges/2, Nmin, 1,'FaceColor', EI_Color(1,:), 'FaceAlpha', 1);
...
%         view([90 -90])
        ...
        title(titleFig7gx{(cond+1)/2*(numel(baseStim)-1)+(stim-numel(baseStim))},'FontSize',18);
        if saveFigs == true
            savefig(strcat(savePath, saveFig7gx{(cond+1)/2*(numel(baseStim)-1)+(stim-numel(baseStim))}));
            title('');
            saveas(gcf, strcat(savePath, saveFig7gx{(cond+1)/2*(numel(baseStim)-1)+(stim-numel(baseStim))}(1:end-3), 'png'));
            saveas(gcf, strcat(savePath, saveFig7gx{(cond+1)/2*(numel(baseStim)-1)+(stim-numel(baseStim))}(1:end-4)), 'epsc');
        end
