...
if totalStim == 6
    titleFig31cx = {'Normalized baseline 100% visual stim. vs 100% visual + photostim. all cells',...
        'Normalized baseline 0% visual stim. vs 0% visual + photostim. all cells norm'};
    
    saveFig31cx = {'meanNormBaseline100BarOIsel.fig','meanNormBaseline0BarOIsel.fig'};
elseif totalStim ==1
    titleFig31cx = {'Normalized baseline 100% visual stim. vs 100% visual + photostim. all cells',...
    ...
    saveFig31cx = {'meanNormBaseline100BarOIsel.fig', 'meanNormBaseline50BarOIsel.fig','meanNormBaseline25BarOIsel.fig','meanNormBaseline12BarOIsel.fig','meanNormBaseline0BarOIsel.fig'};
end
stim =4;
fw = 1;
xval = (1:0.9:3)./fw;
...
%     h1 = line([1.7 4.3],[yl(2)*0.99 yl(2)*0.99]);    
    ...
    if saveFigs == true
        savefig(strcat(savePath, saveFig31cx{(cond+1)/2}));
        title('');
        saveas(gcf, strcat(savePath, saveFig31cx{(cond+1)/2}(1:end-3), 'png'));
        saveas(gcf, strcat(savePath, saveFig31cx{(cond+1)/2}(1:end-4)), 'epsc');
    end
end