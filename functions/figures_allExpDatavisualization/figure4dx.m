...
if totalStim == 6
    titleFig4dx = {'Normalized baseline 100% visual stim. vs 100% visual + photostim. all cells',...
        'Normalized baseline 0% visual stim. vs 0% visual + photostim. all cells norm'};
    
    saveFig4dx = {'boxplotNormBaseline100.fig','boxplotNormBaseline0.fig'};
elseif totalStim ==1
    titleFig4dx = {'Normalized baseline 100% visual stim. vs 100% visual + photostim. all cells',...
    ...
    saveFig4dx = {'boxplotNormBaseline100.fig', 'boxplotNormBaseline50.fig','boxplotNormBaseline25.fig','boxplotNormBaseline12.fig','boxplotNormBaseline0.fig'};
end
stim = 4;
fw = 0.8;
xval = [1,2];
...
for cond = totalConds-1% (1:2:totalConds)
    ...
    b4b = boxplot(yval, xval, 'Colors', 'k', 'Symbol', 'o','Widths',fw,'Notch', 'off')%'BoxStyle','filled');%'none', 'BarWidth', 1/fw); hold on
    ...
    if saveFigs == true
        savefig(strcat(savePath, saveFig4dx{(cond+1)/2}));
        title('');
        saveas(gcf, strcat(savePath, saveFig4dx{(cond+1)/2}(1:end-3), 'png'));
        saveas(gcf, strcat(savePath, saveFig4dx{(cond+1)/2}(1:end-4)), 'epsc');
        writetable(table_fig, strcat(savePath, saveFig4dx{(cond+1)/2}(1:end-3), 'xlsx'),'Sheet',1, 'Range','D:K');
        writetable(table_data1, strcat(savePath, saveFig4dx{(cond+1)/2}(1:end-3), 'xlsx'),'Sheet',1, 'Range','A:C')
    end
end