...
for cond = (1:totalConds) % for all conds
    for code = (1:numel(statsCodesIndMua)) % for all selected good codes 
        ...
        for time = (2:numel(baseTime))
            % compare conditions with photostim to conditions without photostim
            a = squeeze(baselineByTrialMua(cond, statsCodesIndMua(code), keepTrialsBaseMua(cond,code,:),1));
            b = squeeze(baselineByTrialMua(cond, statsCodesIndMua(code), keepTrialsBaseMua(cond,code,:),time));
            if ~(sum(isnan(a)) == numel(a) || sum(isnan(b)) == numel(b))
