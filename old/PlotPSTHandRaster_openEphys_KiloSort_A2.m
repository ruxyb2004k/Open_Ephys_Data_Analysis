...
selectedCodesIndUser = [];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% respMat = ones(1, numel(goodCodes))*3;
if ~isempty(selectedCodesIndUser)
    ...
    selectedCodesIndNew = selectedCodesInd(~ismember(selectedCodesInd, selectedCodesIndSpontNew));
...
elseif ~exist('selectedCodesIndNew', 'var')
    ...
    selectedCodesInd = sort([selectedCodesIndNew, selectedCodesIndSpontNew]);
    selectedCodesIndSpont = double(ismember(selectedCodesInd, selectedCodesIndSpontNew));
end    
...
for code = selectedCodesInd%(1:numel(goodCodes))%(20:22) % plot figures for good or selected codes
    ...
    r.p = uicontrol('style','pushbutton','units','normalized',...
                'position',[0.9, 0.71, 0.08, 0.06],'FontUnits', 'normalized',...
                'FontSize', 0.3,'string','OK');        
    set(r.p, 'callback', @(src, event) p_call2(src, event, r, goodCodes(code)));%, selectedCodesIndNew, selectedCodesIndSpontNew));   
...
%%
selectedCodesInd = sort([selectedCodesIndNew, selectedCodesIndSpontNew]);
selectedCodesIndSpont = double(ismember(selectedCodesInd, selectedCodesIndSpontNew)); % probably redundant
...
if ~exist('selectedCodesIndMuaNew', 'var')
    selectedCodesIndMua = [1:5];%(1:numel(muaCodes)); % selected codes indices
else 
    selectedCodesIndMua = selectedCodesIndMuaNew;
end    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for code = selectedCodesIndMua%(1:numel(muaCodes))%(20:22)%
    rM.f = figure;
    bgM = uibuttongroup('Visible','on','units','pixels',...
                  'Position',[5 2 190 40]);%,...
                  %'SelectionChangedFcn',@bselection);
    rM.c(1) = uicontrol(bgM,'style','radiobutton','units','pixels',...
                'position',[5,20,60,15],'string','VisEv', 'value', 0);
    rM.c(2) = uicontrol(bgM, 'style','radiobutton','units','pixels',...
                'position',[135,20,60,15],'string','none', 'value', 1);         
    rM.p = uicontrol('style','pushbutton','units','pixels',...
                'position',[70,3,70,20],'string','OK');
    set(rM.p, 'callback', @(src, event) p_callMua(src, event, rM));
...
%%
selectedCodesIndMua = selectedCodesIndMuaNew;
b = sort(selectedCodesIndMua);
selectedCodesIndMua = b;
...
figure
for cond = (1:totalConds)
    plot((plotBeg:bin:plotEnd-bin), meanTraceMua(cond, :), 'Color', C(fix((cond+1)/2)), 'LineWidth', mod(cond,2)+2); hold on
end
box off
xlabel('Time [sec]');
ylabel('Count');
