...
clearvars -except med range1 channelNo dataPoints timeSeries sessionInfo filenameSessionInfo filenameTimeSeries experimentName sessionName basePathKilosort basePathData chOffset dataEv timestampsEv ts 
...
for j = 1:channelNo    
    ...
    data_filt_1(1,:) = highpass(data_ch(1,range1), 150, sessionInfo.rates.wideband); % highpass 150 Hz
    m = max([max(data_filt_1(1,:)), abs(min(data_filt_1(1,:)))]);
    gainCh(j)=suggestGain(m); % initial gain for each channel
    data_filt(j,:) = int16(data_filt_1(1,:)*gainCh(j));
end
gain = min(gainCh);
