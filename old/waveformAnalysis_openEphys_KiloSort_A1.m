...
%     end
    ...
    %check for inverted spikes
    [wfMin, IndMin] = min(waveformFiltAvgCh(:,waveformCodeChannel+1));
    [wfMax, IndMax] = max(waveformFiltAvgCh(:,waveformCodeChannel+1));
    
    %Comment the following lines to remove inverting waveforms
%     if wfMax > -wfMin && IndMax<IndMin
%         waveformFiltAvgCh = -waveformFiltAvgCh;
%         waveformDataFilt = -waveformDataFilt;
%         invWf(ind) = true;
%         disp(['Inverted waveform: ', num2str(waveformCode)]);
%     end
...
%     waveformFiltAvg(ind,:) = waveformFiltAvgCh(:,waveformCodeChannelNew(ind)+1); % average waveform for the selected code (single unit) and best channel
    waveformFiltAvg(ind,:) = waveformFiltAvgCh(:,waveformCodeChannel+1); % average waveform for the selected code (single unit) and channel suggested by phy
    ...
    for i = (1:numel(waveformDataPoints)) % store amplitude of each spike - scatter plot with time between consecutive spikes 
        indivTrough(i) = min(waveformDataFilt(i, datapointTrough(ind)-3:datapointTrough(ind)+3, waveformCodeChannelNew(ind)+1));
