...
experimentName = '2021-04-08_11-40-42_trial'
sessionName = 'V1_20210408_1'
...
recordingDepth = [-480 -490]'; % !!! Modify for each experiment !!!
channelNo = 8;
probe = '2x32_H6';%'2x16_P1';% '1x16_P1' '2x16_E1'
animal.name = '20210304_RV1';
animal.sex = 'm';
animal.strain = 'PvCre';
animal.virus = 'AAV9-mOp2A';
recRegion = animal.name(end-2:end);%e.g., 'RV1', 'LV1';
chOffset = 0; % 0 for 16- or 64-channel probes; 16 for 32-channel probe
...
data= int16(zeros(1,dataPoints));
for i=(1:channelNo)
    clearvars data_ch timestamps
    filename = ['100_CH', num2str(i+chOffset), '.continuous'];
    [data_ch, timestamps(:,1), info(:,i)] = load_open_ephys_data_faster([basePathData, filesep, filename], 'unscaledInt16');
%     disp(numel(timestamps))
    data(i,ismember(timestamps1, timestamps)) = data_ch(ismember(timestamps, timestamps1))'*gain_OE;% in case a channel is missing data points, this command will align its data with the first loaded channel
    indx = indx & ismember(timestamps1, timestamps);%%%%
    % calculate median over each channel and subtract from channel
    med(i) = nanmedian(double(data(i,:)));
    std_ch(i) = nanstd(double(data(i,:)));
    data(i,:) = data(i,:) - med(i);
end
...
ts = timestamps1(indx);
timestamps = ts;
data = data(:, indx); %%%% should be uncommented for deleteArtefact
...
selCh = 15; % selected channel for figure and calculation 
...
clearvars -except data range1 channelNo dataPoints timeSeries sessionInfo filenameSessionInfo filenameTimeSeries experimentName sessionName basePathKilosort basePathData chOffset dataEv timestampsEv ts 
...
%     data = double(data1(j,:));
    ...
    data_filt_1 = highpass(data, 150, rf); % highpass 150 Hz
...
for j = 1:channelNo % gain correction 
    if gainCh(j) ~= gain
        j
        data_filt(j,:) = data_filt(j,:)/(gainCh(j)/gain);
    end    
end    
