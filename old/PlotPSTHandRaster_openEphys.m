...
savePath = pwd;
saveFigs = false;
plotBeg=-preTrialTime;
plotEnd=trialDuration;
conditionFieldnames = fieldnames(spikeTimes); % extract condition names

stimTime = round((visStim+preTrialTime)/bin+1);%[17 32 47 62 77 92];
baseTime = round((visStim+preTrialTime-1)/bin+1);%[12, 27, 42, 57, 72, 87];

goodCodes = [];
muaCodes = [];
noiseCodes = [];
for i =(1:numel(spikeClusterData.uniqueCodes(:,1)))
    if spikeClusterData.uniqueCodesLabel(i,1) == 3 % 0 = noise, 1= mua, 2= good
        goodCodes(end+1,1) = spikeClusterData.uniqueCodes(i,1);
    elseif spikeClusterData.uniqueCodesLabel(i,1) == 1 % 0 = noise, 1= mua, 2= good
        muaCodes(end+1,1) = spikeClusterData.uniqueCodes(i,1);
    elseif spikeClusterData.uniqueCodesLabel(i,1) == 0 % 0 = noise, 1= mua, 2= good
...
if narrow == true
    for trialInt= 1:totalTrials
        for rep=1:totalRep % for each repetition
            spikeInTrials1=[spikeInTrials1; (spikeTimes.c100visStim{trialInt, rep})]; % for first cond
            spikeInTrials2=[spikeInTrials2; (spikeTimes.c100optStim{trialInt,rep})]; % for 2nd cond
            spikeInTrials3=[spikeInTrials3; (spikeTimes.c0visStim{trialInt,rep})]; % for 3rd cond
            spikeInTrials4=[spikeInTrials4; (spikeTimes.c0optStim{trialInt,rep})]; % for 4th cond
            %spikeInTrials4=[spikeInTrials4; (spikeTimes.c50visStim{s,rep})];
            %spikeInTrials5=[spikeInTrials5; (spikeTimes.c50optStim{s,rep})];
        end
    end
else
...
selectedCodesIndMua = [1,4,6,8,9];%(1:numel(muaCodes)); % selected codes indices
