...
filenameLFP = fullfile(basePathMatlab,[sessionName,'.lfp.mat']); % general info about the session
...
%     data(j,:) = data_1; % select all the data, regardless of the time range   
    tic
    data(:,j) = lowpass(data_1(:,1), fltrFreq, samplingRate);
    %     data(1:2000000,j) = bandpass(data_1(1:2000000,1), fltrFreqInt, samplingRate);
%     [data_2(1:200000,1),d] = highpass(data_1(1:200000,1), fltrFreqInt(1), samplingRate);
    toc
%     data(1:200000,j) = lowpass(data_2(1:200000,1), fltrFreqInt(2), samplingRate);
...
clearvars P P_allCh cP cWave F T
...
cond = 0;
for chInd = 1%1:numel(channelsToLoad) %(1:channelNo)
    ...
    for i = find(condData.codes == cond)'%8%totalEpochs
        if ismember(i, timeSeries.subTrialsForAnalysis)
            j = j+1;
%             cWave = lfp_ch(1,ceil(recStartDataPoint(i)/downSampleF):floor((recStartDataPoint(i+1)-1)/downSampleF));
            ...
            L = numel(cWave_short);             % Length of signal
            y = fft(cWave_short);
            P2 = abs(y/L);
            P1 = P2(1:L/2+1);
    ...
    P_shortMean(chInd,:) = squeeze(mean(P_short(chInd,:,:), 2));
    
%     figure;
%     plot(f(1:400),smooth(P_shortMean2(chInd,1:400),1)); hold on
%     plot(f(1:400),smooth(P_shortMean(chInd,1:400),1))
...
lfp.FFT.P_shortMean = P_shortMean;
lfp.FFT.P_allCh = P_allCh;


%%
selCh = 32;
P = squeeze(P_allCh(selCh, :, :));

FFTparams.window  = window;             % Window size for computing the spectrogram (FFT) [# samples]
FFTparams.overlap = overlap;             % Overlap of the windows for computing the spectrogram [# samples]
FFTparams.nFFT    = nFFT;          % Vector defining the frequencies for computing the FFT
FFTparams.Fs      = Fs;              % Signal sampling frequency.
...
L = numel(lfp_ch);             % Length of signal
y = fft(lfp_ch);
P2 = abs(y/L);
P1 = P2(1:L/2+1);
P1(2:end-1) = 2*P1(2:end-1);
