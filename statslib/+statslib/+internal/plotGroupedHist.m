...
%   Example:
%   load fisheriris
%   statslib.internal.plotGroupedHist(meas(:,1),species,'color','bcr');
%   hg = statslib.internal.plotGroupedHist(meas(:,1),species);
...
[grpID,gname] =  statslib.internal.grp2idx(group);
...
defaults   = {[],           hsv(nGrp), true,     [],      true};

[h,clr,isAxisOn,nbins,plotGroup,~,extra] = statslib.internal.parseArgs(paramNames,defaults,varargin{:});
if isempty(h)
   h = gca; 
end
...
if isempty(nbins) && nGrp ~= 1
    ...
    xnotinf = x(~isinf(x));
    rx = max(xnotinf) - min(xnotinf);
    if rx/bw > 20
        bw = rx/20; % limit the max number of bins(20)
    end
end
...
if nGrp == 1 
    if isempty(nbins)
        % Change x to x(:) as histogram automatically looks to link a
        % variable 'x' (later 'xg') with the data.
...
clr = statslib.internal.cycleLineProperties(nGrp,clr);
